...
export function Select({
  ...
  const menu = open && menuStyle && (
    <div
      ...
      style={menuStyle}
      className="overflow-auto rounded-xl border border-slate-200 bg-white p-1 text-sm shadow-lg ring-1 ring-slate-900/5 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:ring-white/5"
    >
      {list.length === 0 ? (
        <div className="rounded-lg px-3 py-2 text-slate-400 dark:text-slate-500">Aucune option</div>
      ) : (
        list.map((opt) => (
          <div
